...
// ClientStore is a local storage interface for client data, such as client keys,
// client certificates, trusted host certificates, session data, etc.
//
// ClientStore is split up into three different store types to support mixing different
// backends together. For example, when using `tsh --add-keys-to-agent=only`, ClientStore
// will be made up of an FS (~/.tsh) ProfileStore/TrustedCertsStore and an in-memory KeyStore.
...
type TrustedCertsStore interface {
	// AddTrustedCerts adds the given trusted CA TLS certificates and SSH host keys to the keystore.
...
// HybridClientStore is a local storage interface for client data, such as client keys,
// client certificates, trusted host certificates, session data, etc.
//
// ClientStore is split up into three different store types to support mixing different
// backends together. For example, when using `tsh --add-keys-to-agent=only`, ClientStore
...
func NewHybridClientStore(ks KeyStore, ns TrustedCertsStore, ps ProfileStore) ClientStore {
...
// FSClientStore implements LocalKeyStore interface using the filesystem.
//
...
// NewFSClientStore creates a new filesystem-based local keystore object
// and initializes it.
//
...
func (fs *FSClientStore) AddKey(key *Key) error {
	...
	if err := fs.AddTrustedCerts(key.ProxyHost, key.TrustedCA); err != nil {
		return trace.Wrap(err)
	}
...
func (fs *FSClientStore) GetKey(idx KeyIndex, opts ...CertOption) (*Key, error) {
	...
	trustedCerts, err := fs.getTrustedCerts(idx.ProxyHost, idx.ClusterName)
	if err != nil {
		fs.log.Error(err)
		return nil, trace.ConvertSystemError(err)
	}
...
func (fs *FSClientStore) getTrustedCerts(proxyHost, clusterName string) ([]auth.TrustedCerts, error) {
	tlsCA, err := fs.GetTrustedCertsPEM(proxyHost)
	if err != nil {
		fs.log.Error(err)
		return nil, trace.ConvertSystemError(err)
	}
	sshCAKeys, err := fs.getAuthorizedKeys(proxyHost, proxyHost)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return []auth.TrustedCerts{{
		ClusterName:      clusterName,
		TLSCertificates:  tlsCA,
		HostCertificates: sshCAKeys,
	}}, nil
}
...
// MemClientStore is an in-memory session keystore implementation.
...
// NewMemClientStore initializes a MemLocalKeyStore.
...
func (ms *MemClientStore) AddKey(key *Key) error {
	...
	ms.keys[key.ProxyHost][key.Username][key.ClusterName] = key

	// Save the keys trusted certs to the store.
	if err := ms.AddTrustedCerts(key.ProxyHost, key.TrustedCA); err != nil {
		return trace.Wrap(err)
	}
...
func (ms *MemClientStore) GetKey(idx KeyIndex, opts ...CertOption) (*Key, error) {
	...
	if idx.ClusterName == "" {
		...
		for _, found := range ms.keys[idx.ProxyHost][idx.Username] {
			key = found
			break
		}
	} else {
		key = ms.keys[idx.ProxyHost][idx.Username][idx.ClusterName]
	}
	if key == nil {
