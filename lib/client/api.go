...
func (c *Config) SaveProfile(makeCurrent bool) error {
	...
	if err := c.KeyStore.SaveProfile(profile, makeCurrent); err != nil {
		return trace.Wrap(err)
	}
	return nil
}
...
func NewClient(c *Config) (tc *TeleportClient, err error) {
	...
	if c.KeyStore == nil {
		...
		if c.SkipLocalAuth {
			...
			localAgentCfg.Keystore = noClientStore{}
		} else {
			if c.AddKeysToAgent == AddKeysToAgentOnly {
				localAgentCfg.Keystore = NewMemClientStore()
			} else {
				fsLocalKeyStore, err := NewFSClientStore(c.KeysDir)
				if err != nil {
					return nil, trace.Wrap(err)
				}
