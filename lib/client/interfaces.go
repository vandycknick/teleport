...
func KeyFromIdentityFile(path string) (*Key, error) {
	...
	if len(ident.CACerts.TLS) > 0 || len(ident.CACerts.SSH) > 0 {
		...
		for _, certPEM := range ident.CACerts.TLS {
			cert, err := tlsca.ParseCertificatePEM(certPEM)
			if err != nil {
				return nil, trace.Wrap(err, "identity file contains invalid TLS CA cert")
			}
		...
		var trustedCAs []auth.TrustedCerts
		for clusterName, trustedCA := range clusterCAs {
			// Find all known hosts for the same cluster name.
			hostKeys, err := sshutils.ParseKnownHosts(ident.CACerts.SSH, clusterName)
			if err != nil {
