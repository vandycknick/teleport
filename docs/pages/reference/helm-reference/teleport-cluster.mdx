...
This includes deploying proxies, auth servers, kubernetes-access.
See the [Teleport HA Architecture page](../../deploy-a-cluster/high-availability.mdx) for more details.
...
The `teleport-cluster` chart runs three Teleport services split in two sets of pods:

| Teleport service | Running in | Purpose | Documentation |
| - | - | - | - |
| `auth_service` | auth `StatefulSet` | Authenticates users and hosts, and issues certificates. | [Auth documentation](../../architecture/authentication.mdx)
| `kubernetes_service` | auth `StatefulSet` | Provides secure access to the Kubernetes cluster where the Teleport cluster is hosted. | [Kubernetes Access documentation](../../kubernetes-access/introduction.mdx) |
| `proxy_service` | proxy `Deployment` | Runs the externally-facing parts of a Teleport cluster, such as the web UI, SSH proxy and reverse tunnel service. | [Proxy documentation](../../architecture/proxy.mdx) |
...
| `scratch` (v12 and above) | Generates empty Teleport configuration. User must pass its own config.  | [Running a Teleport cluster with a custom config](../../deploy-a-cluster/helm-deployments/custom.mdx) |

<Admonition type="note" title="Custom mode removal">
`custom` mode has been removed with Teleport version 12. See the [version 12 migration guide]().
</Admonition>
...
to always run to deploy a Teleport version higher or equal than the chart version.
</Admonition>
...
be a fully-qualified domain name (e.g. `teleport.example.com`) as this values is
...
<Admonition type="note">
  ...
  either the IP or hostname of the Kubernetes load balancer.
...
<Admonition type="warning">
  The clusterName must not change in the Teleport cluster lifespan.
</Admonition>
...
The `teleport-cluster` charts deploys two sets of pods: auth and proxy.
`proxy` contains values specific for the auth pods. You can use it to
...
replicated and scheduled. This allows to run Teleport in a highly-available
...
ACME client (opposed to using cert-manager or passing certs through a secret)
...
<Admonition type="warning" title="Version Compatibility">
  If you set a value greater than 1, you **must** meet the replication criteria.
