...
If you are running an older Teleport version, use the version selector to choose
the correct version.
</Admonition>
...
- The `auth` pod configuration should contain at least the `auth_service` section and the `teleport` one.
