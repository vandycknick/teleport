...
```code
$ kubectl --namespace teleport-cluster exec statefulset/teleport-cluster-auth -- tctl users add tadmin --roles=access,editor --logins=ubuntu
...
```code
$ kubectl --namespace=teleport-cluster exec -i statefulset/teleport-cluster-auth -- tctl create -f < member.yaml
```
