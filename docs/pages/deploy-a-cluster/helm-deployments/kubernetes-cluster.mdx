...
<Tabs>
  <TabItem label="Open Source">
    ```code
    # Create a role
    $ kubectl exec -i statefulset/teleport-cluster-auth -- tctl create -f < member.yaml

    # Generate an invite link for the user.
    $ kubectl exec -ti statefulset/teleport-cluster-auth -- tctl  users add alice --roles=member
  ...
  <TabItem label="Enterprise" scope={["enterprise"]}>
    ```code
    # Create a role
    $ kubectl exec -i statefulset/teleport-cluster-ent-auth -- tctl create -f < member.yaml

    # Generate an invite link for the user.
    $ kubectl exec -ti statefulset/teleport-cluster-ent-auth -- tctl  users add alice --roles=member
...
# pod/teleport-auth-0                   1/1     Running   0          127m
# pod/teleport-auth-1                   1/1     Running   0          127m
# pod/teleport-proxy-69c9c4c986-j9v2j   1/1     Running   0          127m
# pod/teleport-proxy-69c9c4c986-qhh5f   1/1     Running   0          127m
```
...
<Tabs>
  <TabItem label="Open Source">
    ...
    $ kubectl exec -i statefulset/teleport-cluster-auth -- tctl create -f < github.yaml
  ...
  <TabItem label="Enterprise"  scope={["enterprise"]}>
    ```code
    $ kubectl exec -i statefulset/teleport-cluster-ent-auth -- tctl create -f < okta.yaml
...
>
  ...
  ```code
  $ kubectl exec -ti statefulset/teleport-cluster-auth -- tail -n 100 /var/lib/teleport/log/events.log
