...
- `custom` mode have been removed as it was broken by the topology change.
...
The upgrade should not require configuration changes.
Make sure you don't rely on `standalone.*` for storage configuration (if you do, switch
to using `persistence` values instead)
...
Version 12 has introduced a new way to pass arbitrary configuration to Teleport without having to
...
configuration overrides through the `auth.teleportConfig` and `proxy.teleportConfig` values.
...
- The `auth` configuration must contain at least the `auth_service` section and the `teleport` section.

The chart automatically creates a Kubernetes join token named after the Helm release which will enable the proxy pods to seamlessly connect to the auth pods.
If you do not want to use this automatic token, you must provide a valid Teleport join token in the proxy pods' configuration.
...
proxy:
  teleportConfig:
    ...
    teleport:
      ...
      join_params:
        method: kubernetes
        token_name: "RELEASE-NAME-proxy"  # replace RELEASE-NAME with the Helm release name
...
joinParams:
  method: token
  tokenName: PUT-YOUR-JOIN-TOKEN-HERE

# Roles can be empty if you pass all the configuration through `teleportConfig`
roles: ""

# Put all your previous `teleport.yaml` except the `teleport` section
teleportConfig:
  # kubernetes_service:
  #   enabled: true
  #   [...]
