...
teleport-cluster-auth-0                   1/1     Running   0          46s
teleport-cluster-proxy-69c9c4c986-j9v2j   1/1     Running   0          46s
teleport-cluster-proxy-69c9c4c986-qhh5f   1/1     Running   0          46s
```
...
teleport-cluster-auth   ClusterIP      10.107.218.220   <none>        3025/TCP,3026/TCP                                                           6m18s
```
...
```code
$ kubectl exec -it statefulset/teleport-cluster-auth -- tctl users add --roles=access appuser
...
```code
$ kubectl exec -it statefulset/teleport-cluster-auth -- bash -c "cat<<EOF>>/home/cp.yaml
...
$ kubectl exec -it statefulset/teleport-cluster-auth -- tctl tokens add \
--type=app \
--app-name=kube-dash \
...
```code
$ kubectl exec -it statefulset/teleport-cluster-auth -- tctl tokens ls
```
